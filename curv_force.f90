...
subroutine tension(N,psi,inter,Frep,ten)
  ...
  do i = 2,N-1
    ...
    b(i-1)  = b(i-1) - sigma*ps1(i)*ps2(i)*(2.d0+1.d0/beta);
  end do
  ! Boundary condition at s=0; dT/ds = (sigma-3*ps2*ps1)
  T2(1) = T2(1) + (4.d0/3.d0)/h2;
  T3(1) = T3(1) - (1.d0/3.d0)/h2;
  b(1)  = b(1)  + (2.d0*h/3.d0)*(gamma-sigma*ps1(1)**2-3.d0*ps1(1)*ps2(1))/h2

  ! Solve using thomas algorithm decomposition
  call tridag(T1,T2,T3,b,Ttemp,N-2)
  ! Substitte
  ten(2:N-1) = Ttemp;
  ten(1)     = (4.d0*ten(2)-ten(3))/(3.d0) -  &
            & (2.d0*h/3.d0)*(gamma-sigma*ps1(1)**2-3.d0*ps1(1)*ps2(1))
  ten(N)     = 0.d0;
...
subroutine feul(N,psi,ten,inter,Fprev,psinew)
  ...
  b = 0.d0; Amat = 0.d0;
  do i = 3,N-2
    b(i) = (Ts(i)*ps1(i)+3.d0*ps2(i)*ps1(i)**2)*(1.d0+beta) - beta*gamma*ps1(i)
    b(i) = b(i) + cperp*(Ut(i)*ps1(i)+Uns(i));
    b(i) = b(i) + sigma*beta*ps1(i)**3
    Fprev(i) = b(i);
    b(i) = b(i)*factor + psi(i);


    Amat(i,i-2) = 1.d0*factor/h4  - 0.5d0*sigma*factor/h3;
    Amat(i,i-1) =-4.d0*factor/h4 -      ten(i)*factor/h2 + sigma*factor/h3;
  ...
  ! EBC3
  b(1)       = -ps1(1)**3;
  Amat(1,1)  = (-3.d0*ten(1)/(2.d0*h)-(-2.5d0/h3))-sigma*( 2.d0/h2);
  Amat(1,2)  = ( 4.d0*ten(1)/(2.d0*h)-( 9.0d0/h3))-sigma*(-5.d0/h2);
  Amat(1,3)  = (-1.d0*ten(1)/(2.d0*h)-(-12.d0/h3))-sigma*( 4.d0/h2);
  Amat(1,4)  = (0.d0                 -( 7.0d0/h3))-sigma*(-1.d0/h2);
...
subroutine beul(N,delt,ten,inter,Frep,Fprev,psiprev,psinew)
  ...
  b = 0.d0; Amat = 0.d0;
  do i = 3,N-2
    Fnew(i) = (Ts(i)*ps1(i)+3.d0*ps2(i)*ps1(i)**2)*(1.d0+beta) - beta*gamma*ps1(i)
    Fnew(i) = Fnew(i) + cperp*(Ut(i)*ps1(i)+Uns(i));
    Fnew(i) = Fnew(i) - (Fns(i)+beta*Ft(i)*ps1(i));
    Fnew(i) = Fnew(i) + sigma*beta*ps1(i)**3;
...
subroutine force(N,alpha,psi,ten,fx,fy)
  ...
  do k = 1,alpha
    ...
    do j = 1,N   ! At the jth point
      ! Obtain the tangential and normal force
      fT(j) =  3.d0*ps1(j)*ps2(j) + Ts(j) + sigma*ps1(j)**2-gamma;
      fN(j) = -ps3(j)+ps1(j)**3 + ten(k,j)*ps1(j)-sigma*ps2(j);
